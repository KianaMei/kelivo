...
/// triggering a full page rebuild. Instead of using setState() which causes
/// the entire HomePage to rebuild, this uses ValueNotifier so only the
/// specific message widget that's listening will rebuild.
///
/// Performance improvement: 10 FPS -> 60 FPS during streaming
///
/// Usage:
/// 1. StreamContext updates content via updateContent()
...
class StreamingContentNotifier {
  ...
  /// Each streaming message has its own ValueNotifier<StreamingContentData>.
  ...
  void updateContent(String messageId, String content, int totalTokens, {String? tokenUsageJson}) {
  ...
  void updateReasoning(String messageId, {
    ...
    if (notifier != null) {
      ...
      notifier.value = StreamingContentData(
        ...
        tokenUsageJson: current.tokenUsageJson,
...
class StreamingContentData {
  ...
  bool operator ==(Object other) =>
      ...
      other is StreamingContentData &&
          ...
          tokenUsageJson == other.tokenUsageJson &&
