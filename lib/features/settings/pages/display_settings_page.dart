...
Widget _sheetDivider(BuildContext context) {
  final cs = Theme.of(context).colorScheme; return Divider(height: 1, thickness: 1.0, indent: 52, endIndent: 16, color: cs.outlineVariant.withOpacity(0.18));
}
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 16), children: [
        _iosSectionCard(children: [
          ...
          _iosDivider(context),
          _iosSwitchRow(context, icon: Lucide.Image, label: '显示表情包工具调用', value: sp.showStickerToolUI, onChanged: (v) => context.read<SettingsProvider>().setShowStickerToolUI(v)),
        ]),
      ]),
    );
  }
