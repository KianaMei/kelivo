...
Future<void> showDesktopModelFetchDialog(
  ...
  print('═══════════════════════════════════════════════════════');
  print('🎯 showDesktopModelFetchDialog CALLED!');
  print('   Provider: $providerKey');
  print('   Display Name: $providerDisplayName');
  print('═══════════════════════════════════════════════════════');
...
class _ModelFetchDialogBodyState extends State<_ModelFetchDialogBody> {
  ...
  Future<void> _load() async {
    print('🚀 [ModelFetch] _load() started for provider: ${widget.providerKey}');
    final settings = context.read<SettingsProvider>();
    final rawCfg = settings.getProviderConfig(widget.providerKey, defaultName: widget.providerDisplayName);
    // Apply URL sanitization before API call
  ...
  Widget build(BuildContext context) {
    ...
    // Compute filtered list and selection state for toggle icon
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          icon: Icon(Lucide.X, size: 20, color: cs.onSurface.withOpacity(0.9)),
                        ),
                      ],
                    ),
                  ),
                              ...
                              suffixIcon: Row(
                                ...
                                children: [
                                  Tooltip(
                                    ...
                                    child: AnimatedSwitcher(
                                      ...
                                      child: IconButton(
                                        ...
                                        icon: Icon(
                                          allHeaderFilteredSelected ? Lucide.Square : Lucide.CheckSquare,
                                          size: 18,
                                  ...
                                  Tooltip(
                                    ...
                                    child: IconButton(
                                      icon: Icon(Lucide.Repeat, size: 18, color: cs.onSurface.withOpacity(0.7)),
                                      padding: EdgeInsets.zero,
                                      constraints: const BoxConstraints(minWidth: 40, minHeight: 36),
                                      onPressed: () async {
                                        final settings = context.read<SettingsProvider>();
                    ...
                    child: Row(
                      children: [
                        SizedBox(
                          ...
                          child: Center(
                            child: AnimatedRotation(
                              ...
                              child: Icon(Lucide.ChevronRight, size: 18, color: cs.onSurface.withOpacity(0.7)),
                            ),
                          ),
                        ),
                        ...
                        IconButton(
                          ...
                          padding: EdgeInsets.zero,
                          constraints: const BoxConstraints(minWidth: 40, minHeight: 36),
                          icon: Icon(allAdded ? Lucide.Minus : Lucide.Plus, size: 18, color: cs.onSurface.withOpacity(0.75)),
                          onPressed: () async {
  ...
  Widget _buildModelRow(BuildContext context, ModelInfo m, {bool isUnavailable = false}) {
    ...
    // Vision input (Eye icon)
    if (m.input.contains(Modality.image)) {
      caps.add(pillCapsule(Icon(Lucide.Eye, size: 12, color: cs.secondary), cs.secondary));
    }
    
    // Image output (Image icon)
    if (m.output.contains(Modality.image)) {
      caps.add(pillCapsule(Icon(Lucide.Image, size: 12, color: cs.tertiary), cs.tertiary));
    }
    
    // Abilities: Tool and Reasoning
    for (final ab in m.abilities) {
      if (ab == ModelAbility.tool) {
        caps.add(pillCapsule(Icon(Lucide.Hammer, size: 12, color: cs.primary), cs.primary));
      } else if (ab == ModelAbility.reasoning) {
        caps.add(pillCapsule(
          SvgPicture.asset('assets/icons/deepthink.svg', width: 12, height: 12, colorFilter: ColorFilter.mode(cs.secondary, BlendMode.srcIn)), 
          cs.secondary,
        ));
      }
    }
...
class _BrandAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (asset != null) {
      ...
      final firstChar = name.isNotEmpty ? name.substring(0, 1).toUpperCase() : '?';
      inner = Text(firstChar, style: TextStyle(color: cs.primary, fontWeight: FontWeight.w700, fontSize: size * 0.42));
    }
    return Container(
      width: size,
      height: size,
